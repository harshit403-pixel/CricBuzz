...
function TeamsList() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center">Loading teams...</div>
    );
  }

  if (isError) {
    return <div className="text-red-500">Failed to load teams.</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-800">
        <table className="w-full">
          <thead className="bg-slate-900">
            <tr>
              ...
              <th className="px-4 py-3 text-left">Primary Color</th>
          ...
          <tbody>
            {teams.map((team) => (
              <tr key={team._id} className="border-t border-slate-800">
        ...
        {teams.length === 0 && (
          <div className="p-6 text-center text-slate-400">No teams found.</div>
        )}
      </div>
      <ConfirmModal
        open={!!selectedTeam}
